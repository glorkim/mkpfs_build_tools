...
(titleId, titleName, icon, and userDefinedParam can also be edited during the build process for archived games.)

> No extraction is required at any stage, and the tool is optimized to fully utilize your CPU and memory.
...
### v1.36
- Add AMPR injection checkbox to the build tab
- AMPR index (`ampr_emu.index`): generated by injection at build time from a folder or archive (zip/rar/7z)
- New "AMPR index" tab: drop a folder/archive to create `ampr_emu.index` (kept at the source location)
- Inject `ampr_emu.index` at build time (folder/rar/zip/7z); overwrites an existing index
- Add titleId editing to the param.json editor
- Unpack tab: extract `.exfat` images to a folder (built-in exFAT extractor)
- Renamed the Settings tab to "Folder Settings"

### v1.35
- Add titleName / userDefinedParam editing to build
- Add "Apply param.json (titleName / userDefinedParam)" option
- Auto-remove invalid title characters (™ ® ©) before compression
- GUI language selection (Korean / English, saved to config)
